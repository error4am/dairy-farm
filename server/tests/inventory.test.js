...
function expectFieldError(fn, field, message) {
  assert.throws(fn, (err) => err.status === 400 && Boolean(err.details && err.details[field]), message);
}

function expectStatus(fn, status, message) {
  assert.throws(fn, (err) => err.status === status, message);
}

const silage = inventoryService.create({
  name: 'Wanda Silage',
  category: 'silage',
  unit: 'kg',
  minimum_stock: 2000,
...
test('creates inventory items and enforces unique names', () => {
...
test('rejects invalid item input', () => {
  expectFieldError(() => inventoryService.create({ category: 'silage', unit: 'kg' }), 'name', 'name required');
  expectFieldError(() => inventoryService.create({ name: 'X1', category: 'gold', unit: 'kg' }), 'category', 'category enum');
  expectFieldError(() => inventoryService.create({ name: 'X2', category: 'silage' }), 'unit', 'unit required');
  expectFieldError(
...
test('edits and deactivates items', () => {
  const updated = inventoryService.update(silage.id, {
    name: 'Wanda Silage',
    category: 'silage',
    unit: 'kg',
    minimum_stock: 2500,
  ...
  const inactive = inventoryService.update(silage.id, {
    name: 'Wanda Silage',
    category: 'silage',
    unit: 'kg',
    minimum_stock: 2500,
  ...
  const reactivated = inventoryService.update(silage.id, {
    name: 'Wanda Silage',
    category: 'silage',
    unit: 'kg',
    minimum_stock: 2500,
...
test('records the full movement lifecycle and derives current stock', () => {
  const opening = movementService.create({
    item_id: silage.id,
  ...
  const purchase = movementService.create({
    item_id: silage.id,
  ...
  const consumption = movementService.create({
    item_id: silage.id,
  ...
  const waste = movementService.create({
    item_id: silage.id,
  ...
  const adjustment = movementService.create({
    item_id: silage.id,
  ...
  const item = inventoryService.get(silage.id);
  assert.equal(item.current_stock, 13700, '10000 + 5000 - 1200 - 200 + 100');

  const profile = inventoryService.profile(silage.id);
...
test('supports negative adjustments', () => {
  const before = inventoryService.get(silage.id).current_stock;
  ...
  assert.equal(inventoryService.get(silage.id).current_stock, before - 300);
});

test('rejects invalid movement input', () => {
  expectFieldError(
...
test('blocks movements that would make stock negative', () => {
  const fodder = inventoryService.create({ name: 'Turi Fodder', category: 'fodder', unit: 'kg' });
  movementService.create({ item_id: fodder.id, date: today, type: 'opening', quantity: 100, unit: 'kg' });
  ...
  const consumption = movementService.create({ item_id: fodder.id, date: today, type: 'consumption', quantity: 40, unit: 'kg' });
  expectStatus(
    () =>
  ...
  const purchase = movementService.create({ item_id: fodder.id, date: today, type: 'purchase', quantity: 10, unit: 'kg' });
  const purchaseOnly = inventoryService.create({ name: 'Berseem Green Fodder', category: 'fodder', unit: 'kg' });
  const onlyPurchase = movementService.create({ item_id: purchaseOnly.id, date: today, type: 'purchase', quantity: 50, unit: 'kg' });
  movementService.create({ item_id: purchaseOnly.id, date: today, type: 'consumption', quantity: 50, unit: 'kg' });
  expectStatus(
  ...
  assert.equal(inventoryService.get(purchaseOnly.id).current_stock, 0);
  assert.equal(inventoryService.get(fodder.id).current_stock, 70, '100 - 40 + 10');
  assert.ok(purchase.id > 0);
});

test('inactive items only accept adjustments', () => {
  const supply = inventoryService.create({ name: 'Mineral Mix', category: 'mineral', unit: 'kg' });
  movementService.create({ item_id: supply.id, date: today, type: 'opening', quantity: 100, unit: 'kg' });

  inventoryService.update(supply.id, {
    name: 'Mineral Mix',
    category: 'mineral',
    unit: 'kg',
    active: false
...
test('purchase creates exactly one linked finance expense', () => {
  const concentrate = inventoryService.create({ name: 'Wanda Concentrate', category: 'concentrate', unit: 'kg' });
...
test('purchase total cost is computed from unit cost when missing', () => {
  const item = inventoryService.create({ name: 'Silage Additive', category: 'supply', unit: 'L' });
  const before = financeService.totals();
  ...
  const free = movementService.create({
    item_id: item.id,
    date: today,
    type: 'purchase',
    quantity: 2,
  ...
  assert.equal(financeService.totals().count, after.count, 'no extra expense for zero cost');
});

test('editing a purchase updates the linked expense without duplicates', () => {
  const item = inventoryService.create({ name: 'Maize Silage', category: 'silage', unit: 'kg' });
  const purchase = movementService.create({
    item_id: item.id,
    date: today,
    type: 'purchase',
  ...
  const updated = movementService.update(purchase.id, {
    item_id: item.id,
    date: addDays(today, -1),
    type: 'purchase',
    quantity: 100,
...
test('clearing purchase cost removes the linked expense', () => {
  const item = inventoryService.create({ name: 'Cotton Seed Cake', category: 'concentrate', unit: 'kg' });
  const purchase = movementService.create({
    item_id: item.id,
    date: today,
    type: 'purchase',
  ...
  const cleared = movementService.update(purchase.id, {
    item_id: item.id,
    date: today,
    type: 'purchase',
    quantity: 50,
  ...
  assert.equal(financeService.totals().count, beforeCount - 1, 'expense removed with the cost');
});

test('deleting a purchase removes its linked expense', () => {
  const item = inventoryService.create({ name: 'Wheat Bran', category: 'concentrate', unit: 'kg' });
  const purchase = movementService.create({
    item_id: item.id,
    date: today,
    type: 'purchase',
  ...
  const result = movementService.remove(purchase.id);
  assert.equal(result.ok, true);
  assert.equal(financeService.totals().count, beforeCount - 1, 'expense deleted with the purchase');
  assert.equal(db.prepare('SELECT COUNT(*) AS n FROM transactions WHERE id = ?').get(txId).n, 0);
  assert.equal(inventoryService.get(item.id).current_stock, 0);
});

test('finance cannot edit or delete linked inventory expenses', () => {
  const item = inventoryService.create({ name: 'Urea Treatment Molasses', category: 'supply', unit: 'L' });
  const purchase = movementService.create({
    item_id: item.id,
    date: today,
    type: 'purchase',
...
test('manually deleting the linked transaction lets an edit recreate it', () => {
  const item = inventoryService.create({ name: 'Salt Lick', category: 'mineral', unit: 'kg' });
  const purchase = movementService.create({
    item_id: item.id,
    date: today,
    type: 'purchase',
  ...
  assert.equal(movementService.get(purchase.id).transaction_id, null, 'FK sets the link to null');

  const recreated = movementService.update(purchase.id, {
    item_id: item.id,
    date: today,
    type: 'purchase',
    quantity: 5,
...
test('movement listing supports filters, ordering and pagination', () => {
  const item = inventoryService.create({ name: 'Filter Test Item', category: 'other', unit: 'kg' });
  movementService.create({ item_id: item.id, date: addDays(today, -3), type: 'opening', quantity: 100, unit: 'kg' });
  movementService.create({ item_id: item.id, date: addDays(today, -2), type: 'purchase', quantity: 50, unit: 'kg', supplier: 'FilterSupplier' });
  movementService.create({ item_id: item.id, date: addDays(today, -1), type: 'consumption', quantity: 10, unit: 'kg' });

  const all = movementService.list({ item_id: item.id });
  assert.equal(all.total, 3);
  assert.equal(all.items[0].type, 'consumption', 'newest movement first');

  const purchases = movementService.list({ item_id: item.id, type: 'purchase' });
  assert.equal(purchases.total, 1);

  const ranged = movementService.list({ item_id: item.id, from: addDays(today, -2), to: addDays(today, -1) });
  assert.equal(ranged.total, 2);

  const searched = movementService.list({ search: 'FilterSupplier' });
  assert.equal(searched.total, 1);

  const paged = movementService.list({ item_id: item.id, limit: 2, offset: 0 });
  assert.equal(paged.items.length, 2);
  assert.equal(paged.total, 3);
  const page2 = movementService.list({ item_id: item.id, limit: 2, offset: 2 });
  assert.equal(page2.items.length, 1);
});

test('low stock and out of stock are derived from movements and thresholds', () => {
  const low = inventoryService.create({ name: 'Low Stock Item', category: 'fodder', unit: 'kg', minimum_stock: 100 });
  movementService.create({ item_id: low.id, date: today, type: 'opening', quantity: 50, unit: 'kg' });

  const healthy = inventoryService.create({ name: 'Healthy Stock Item', category: 'fodder', unit: 'kg', minimum_stock: 100 });
  movementService.create({ item_id: healthy.id, date: today, type: 'opening', quantity: 200, unit: 'kg' });

  const empty = inventoryService.create({ name: 'Empty Stock Item', category: 'fodder', unit: 'kg', minimum_stock: 100 });
  movementService.create({ item_id: empty.id, date: today, type: 'opening', quantity: 10, unit: 'kg' });
  movementService.create({ item_id: empty.id, date: today, type: 'consumption', quantity: 10, unit: 'kg' });

  const inactive = inventoryService.create({ name: 'Inactive Stock Item', category: 'fodder', unit: 'kg', minimum_stock: 100 });
  inventoryService.update(inactive.id, {
  ...
  const lowList = inventoryService.list({ status: 'low' });
  const outList = inventoryService.list({ status: 'out' });
  const summary = inventoryService.summary();
  ...
  assert.equal(summary.active_count, inventoryService.list({ status: 'active' }).length);
});

test('dashboard inventory metrics match the inventory summary', () => {
...
test('items with movements cannot be deleted; empty items can', () => {
  const emptyItem = inventoryService.create({ name: 'Delete Me', category: 'other', unit: 'kg' });
  const result = inventoryService.remove(emptyItem.id);
  assert.equal(result.ok, true);
  expectStatus(() => inventoryService.get(emptyItem.id), 404, 'deleted item is gone');
  ...
  assert.ok(inventoryService.get(silage.id), 'item with history still exists');
});

test('farm scoping keeps other farms data invisible and unusable', () => {
  ...
  assert.ok(!inventoryService.list().some((i) => i.id === foreignId), 'foreign item not listed');
  expectStatus(() => inventoryService.get(foreignId), 404, 'foreign item not readable');
  expectFieldError(
...
test('failed updates leave movements and transactions unchanged', () => {
  const item = inventoryService.create({ name: 'Atomicity Item', category: 'other', unit: 'kg' });
  const purchase = movementService.create({
    item_id: item.id,
    date: today,
    type: 'purchase',
  ...
  const unchanged = movementService.get(purchase.id);
