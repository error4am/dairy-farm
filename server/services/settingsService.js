...
function update(body) {
  ...
  db.prepare(
    `UPDATE farms SET name = ?, currency = ?, milk_unit = ?, week_start = ?, gestation_days = ?, updated_at = datetime('now')
     WHERE id = ?`
  ).run(data.name, data.currency.toUpperCase(), data.milk_unit, data.week_start, data.gestation_days, FARM_ID);
