...
async function get() {
  ...
  const activeAnimals = (
    await db.get("SELECT CAST(COUNT(*) AS INTEGER) AS n FROM animals WHERE farm_id = ? AND status = 'active'", [FARM_ID])
  ).n;

  const todayMilk = await db.get(
    `SELECT
  ...
  const weekMilk = (
    await db.get(
      ...
      [FARM_ID, weekStart, today]
    )
  ).total;

  const finance = await financeService.totals();
  const health = await healthService.summary();
  const breeding = await breedingService.summary();
